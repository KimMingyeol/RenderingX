...
app_function (void *userdata)
{
    ...
    /* Build pipeline */
    data->pipeline =
            gst_parse_launch ("rtspsrc location=rtsp://wowzaec2demo.streamlock.net/vod/mp4:BigBuckBunny_115k.mp4 latency=100 ! queue ! rtph264depay ! h264parse ! avdec_h264 ! autovideosink",
                              &error);
    if (error) {
        gchar *message =
...
gst_native_init (JNIEnv * env, jobject thiz)
{
    ...
    GST_DEBUG_CATEGORY_INIT (debug_category, "gstreamer-rtsp", 0,
                             "Android tutorial 3");
    gst_debug_set_threshold_for_name ("gstreamer-rtsp", GST_LEVEL_DEBUG);
...
gst_native_class_init (JNIEnv * env, jclass klass)
{
    ...
    if (!custom_data_field_id || !on_gstreamer_initialized_method_id) {
         ...
         */
        __android_log_print (ANDROID_LOG_ERROR, "gstreamer-rtsp",
...
JNI_OnLoad (JavaVM * vm, void *reserved)
{
    ...
    jclass klass = (*env)->FindClass (env,
                                      "constantin/renderingx/example/stereo/video360degree/AExample360Video");
